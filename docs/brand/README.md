...
The current desktop icon at `apps/desktop/src-tauri/icons/icon.png` is derived from this selected direction.

Next refinement pass:

- Produce a true multi-size macOS `.icns` once the official Tauri bundle path is finalized.
- Test legibility at 16px, 32px, 64px, and 128px.
- Consider a slightly simpler small-size variant if the right-side nodes become too fine.
